...
MD = ROOT / "output" / "resume" / "Steven_de_la_Torre_Master_Resume.md"
DOCX = ROOT / "output" / "resume" / "Steven_de_la_Torre_Master_Resume.docx"
PDF = ROOT / "output" / "resume" / "Steven_de_la_Torre_Master_Resume.pdf"
TMP = ROOT / "tmp" / "resume"
...
def embedded_pdf_fonts(reader):
    ...
    unique = {(item["base_font"], item["subtype"], item["embedded"]) for item in results}
    return [dict(base_font=a, subtype=b, embedded=c) for a, b, c in sorted(unique)]


def main():
    checks = {}
    for path in (MD, DOCX, PDF):
        checks[f"exists_{path.suffix[1:]}"] = path.exists() and path.stat().st_size > 0

    markdown = MD.read_text(encoding="utf-8")
    checks["markdown_no_placeholders"] = not re.search(
        r"INSERT CURRENT|\bTBD\b|\bTODO\b|drive-fast-take-chances\.com", markdown, re.I
    )
    checks["portfolio_url_current"] = "https://practitioner.digital" in markdown

    document = Document(DOCX)
    docx_text, document_xml, rels_xml = docx_text_and_xml()
    (TMP / "docx-text.txt").write_text(docx_text, encoding="utf-8")
    section = document.sections[0]
    checks["docx_one_section"] = len(document.sections) == 1
    checks["docx_letter_page"] = round(section.page_width.inches, 2) == 8.5 and round(section.page_height.inches, 2) == 11.0
    checks["docx_margins"] = (
    ...
    markdown_bullets = len(re.findall(r"(?m)^- ", markdown))
    numbered_paragraphs = sum(
        1 for paragraph in document.paragraphs if paragraph._p.pPr is not None and paragraph._p.pPr.numPr is not None
    )
    ...
    checks["docx_no_textboxes_or_drawings"] = "w:txbxContent" not in document_xml and "w:drawing" not in document_xml
    ...
    pdf_text = "\n".join(pdf_pages_text)
    (TMP / "pdf-text.txt").write_text(pdf_text, encoding="utf-8")
    checks["pdf_two_pages"] = len(reader.pages) == 2
    checks["pdf_nonblank_pages"] = all(len(re.sub(r"\s+", "", page_text)) > 200 for page_text in pdf_pages_text)
    checks["pdf_letter_pages"] = all(
        round(float(page.mediabox.width)) == 612 and round(float(page.mediabox.height)) == 792 for page in reader.pages
    )
    link_annotations = 0
    for page in reader.pages:
